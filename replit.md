...
- **Consolidated XP Updates**: `useXPUpdater` hook (`client/src/hooks/useXPUpdater.ts`) provides centralized real-time XP management:
  - `updateAllXPDisplays(xpChange, source, options)` - Optimistically updates total XP, level, and XP Today in a single call
  - `deductXP(xpAmount, source, options)` - Handles XP removal (e.g., mission undo)
  - `refreshAllXPData()` - Invalidates all XP-related queries for full refresh
  - `updateStreak(newStreak)` and `updateLives(livesChange)` - Streak and lives management
  - Supports event listeners via `useXPUpdateListener` hook for custom reactions (e.g., confetti animations)
  - Server-side `/api/gamification` returns `xpToday` calculated from all sources: emotional check-ins, completed missions, and medication logs
